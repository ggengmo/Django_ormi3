...
# 목표
1. DRF에 친숙해진다.
2. 기본적ㅈ인 바닐라JS로 값을 주고 받는 것을 확인한다.(중복되는 코드가 많습니다.)
3. 확장성 과제(친구 맺기, 프로필 수정, 친구 검색, 친구들의 피드만 볼 수 있게 하는 것)
###################################
URL 설계
# api
GET     /api/schema/
...
# posts
GET     /posts/ # 글 목록 - 로그인한 사용자
GET     /posts/{id}/ # 글 상세(댓글, 좋아요) - 로그인한 사용자
GET     /posts/{post_id}/comments/ # 댓글 목록 - 로그인한 사용자
POST    /posts/{post_id}/like/ # 좋아요 생성 - 로그인한 사용자
DELETE  /posts/{post_id}/like/ # 좋아요 삭제 - 로그인한 사용자
POST    /posts/comments/ # 댓글 생성 - 로그인한 사용자
POST    /posts/write/ # 글 생성 - 로그인한 사용자


# user
POST    /users/signup/ # 회원가입 - 모두 공개
GET     /users/user/{id}/ # 프로필  - 로그인한 사용자에게 전체 공개
###################################
