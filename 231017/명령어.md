...
{% for i in posts %}
    ...
    <p>말자름: {{i.content|truncatewords:5}}</p>
    <p>슬라이싱: {{i.content|slice:":10"}}
    </p>
...
{% for i in posts %}
    ...
    <p>{{ forloop.counter0 }}번째 게시물입니다.</p>
    <p>{{forloop.counter|add:100}}번째 게시물입니다.</p>
...
###################################
# blog > templates > blog > postlist.html
# Django의 이스케이프 기능 비활성화하는 방법 2가지(특수한 경우에만 사용, 주의 해야함.)
...
# settings.py 'markdown_deux' APP 등록
...
###################################
# blog > templates > blog > post_deatil.html
